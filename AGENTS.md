...
Welcome to the **CommuteBrief / CommuteNews** repository. This project follows a **Spec-Driven Development (SDD)** workflow where documentation and specs are the source of truth for all software development, feature additions, and bug fixes.
...
4. **Validation & Verification**: Run `npm run lint` and `npm run build` to verify compilation and type safety. Cross-check against `/specs/VALIDATION_CHECKLIST.md`.
5. **Context Handoff**: Update `/specs/IMPLEMENTATION_PLAN.md` and `CHANGELOG.md` so future agents or developers can resume work without context loss.
...
- Prefer explicit domain types in `src/types.ts`.
- Avoid raw `any`; use `unknown` and type guards for caught errors.
- Use immutable patterns and `readonly` arrays where appropriate.
- Keep changes minimal and traceable to the spec.
- Do not introduce code that is not covered by the spec or implementation plan.
