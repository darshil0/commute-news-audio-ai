...
**CommuteBrief (CommuteNews)** is a full-stack, audio-first single-page web application (SPA) designed to optimize daily commutes. It accepts pasted text articles or web news URLs, converts them into concise, structured summaries using server-side Gemini AI models, synthesizes natural-sounding audio briefings using AI voice models or browser-native Text-to-Speech (TTS) fallbacks, and provides **cross-device synchronization** for playlists and user briefs powered by Express JWT Authentication, server-side sync endpoints (`/api/sync`), and client-side IndexedDB persistence.
...
6. **Cross-Device Synchronization (Express Sync API)**:
   - Server-side cross-device sync across devices for user briefs (`Article[]`), custom playlists (`Playlist[]`), and user preferences (`UserPreferences`) via `/api/sync/save` and `/api/sync/get`.
   - JWT Authentication (`/api/auth/register`, `/api/auth/login`) anchoring user isolation scope.
   - Offline-first architecture where local IndexedDB operations sync to Express server file storage (`data/sync_{username}.json`) upon network reconnection.
7. **Haptic Feedback**:
...
## 3. Server Sync Architecture & Persistence Spec

### 3.1 Data Hierarchy & Sync Schema (`SyncData`)

Data is stored server-side under `data/sync_{username}.json` and communicated via JSON payloads over `/api/sync/save` and `/api/sync/get`:

#### Entity: `Article` (`UserBrief`)

- **Properties**:
  - `id` (`string`, required): Unique identifier (`crypto.randomUUID()`).
  ...
  - `isSaved` (`boolean`, optional): Bookmarked status flag.
  - `isDownloaded` (`boolean`, optional): Local audio cache flag.

#### Entity: `Playlist` (`UserPlaylist`)

- **Properties**:
  - `id` (`string`, required): Unique playlist identifier (`crypto.randomUUID()`).
  ...
  - `articleIds` (`array` of `string`, max 100 items): Ordered list of `Article` IDs in the playlist.
  - `createdAt` (`string`, required): ISO creation timestamp.

#### Entity: `UserPreferences` (`UserSettings`)

- **Properties**:
  - `voiceName` (`string`, required): Selected voice profile ID (`Zephyr`, `Kore`, `Charon`, `Puck`, `Fenrir`).
  - `playbackSpeed` (`number`, required): Preferred speed factor (`0.5` to `2.0`).
  - `summaryLength` (`string`, required): Desired summary depth (`short`, `medium`, `detailed`).
  - `summaryTone` (`string`, required): Desired summary style (`concise`, `engaging`, `executive`).
  - `theme` (`string`, required): UI theme selection (`dark`, `light`).

---

### 3.2 Security & Authorization Model

The Express server enforces JWT-based authorization and input security:

1. **User Authorization Isolation**: Sync requests require `Authorization: Bearer <token>` verified using server `TOKEN_SECRET`.
2. **Username Safety**: Username allowlist regex (`/^[a-z0-9_-]{3,32}$/`) prevents path traversal attacks when referencing `data/sync_{username}.json`.
3. **Payload Boundary Limits**: Route limiters and JSON body bounds prevent oversized sync submissions.
4. **Local IndexedDB Security**: Local client data is scoped within browser IndexedDB (`commutenews_db`).
...
2. **Bi-Directional Server Sync**:
   - `syncWithServer()` fetches latest backup via `ApiService.getBackupData(token)` and pushes merged data via `ApiService.backupData(token, payload)`.
3. **Conflict Resolution Strategy**: Atomic timestamp-based merging (`createdAt`) guarantees last-write wins without data loss.
4. **Audio Binary Handling**: Raw synthesized audio buffers or audio base64 are stored locally in IndexedDB / Web Audio cache, keeping sync JSON payloads lean.
...
### US-7: Cross-Device Synchronization

- **As a** multi-device commuter (e.g. laptop at work, phone on train),
- **I want** my briefs, playlists, and user preferences to synchronize seamlessly via Express Sync endpoints,
- **So that** I can switch devices and access my saved audio content anywhere.
...
| **AC-7.1** | Sync           | JWT authentication anchors user sync scope and protects `/api/sync/save` and `/api/sync/get` endpoints.                                                                       |
| **AC-7.2** | Sync           | Creating, updating, or deleting briefs/playlists updates local IndexedDB and syncs to server storage via `/api/sync/save`.                                                  |
| **AC-7.3** | Sync           | User preferences and brief data synchronize across authenticated devices via `/api/sync/get` and `/api/sync/save`.                                                          |
| **AC-7.4** | Sync           | Server authorization strictly validates JWT Bearer tokens and username regex allowlists (`/^[a-z0-9_-]{3,32}$/`).                                                            |
...
| **AC-8.5** | Data Integrity | Diagnostic tests purge temporary progress records and test entries to prevent sync data pollution.                                                                           |
...
- **Public Community Brief Sharing**: Public brief social feeds or uncontrolled cross-user data access are excluded; data access is strictly locked to the authenticated owner.
- **Binary Audio Storage in Sync Payload**: Raw audio blob binaries are NOT stored inside sync JSON payloads; audio streams and local audio files are cached in IndexedDB or Web Audio memory to keep sync payloads lightweight.
