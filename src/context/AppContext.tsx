...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const currentSpeechUtteranceRef = useRef<SpeechSynthesisUtterance | null>(
    null,
  );
  const playNextInQueueRef = useRef<() => void>(() => {});
  const sleepTimerIdRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const audioProgressIntervalRef = useRef<ReturnType<
    typeof setInterval
  > | null>(null);
  ...
  const clearPlaybackErrorLater = useCallback((message: string) => {
    ...
    errorTimeoutRef.current = setTimeout(() => {
      setPlaybackState((prev) =>
        prev.playbackError === message
          ? { ...prev, playbackError: null }
          : prev,
      );
    }, 6000);
  }, []);
      ...
      if (remoteData) {
        for (const remoteArt of remoteData.articles) {
          ...
          if (index === -1) {
            ...
            new Date(remoteArt.createdAt).getTime() >
            new Date(mergedArticles[index].createdAt).getTime()
          ) {
            mergedArticles[index] = {
              ...remoteArt,
              isDownloaded:
                mergedArticles[index].isDownloaded || remoteArt.isDownloaded,
            };
    ...
    const initializeData = async () => {
      try {
        ...
        if (savedPrefs) {
          ...
          setPlaybackState((prev) => ({
            ...prev,
            speed: savedPrefs.playbackSpeed,
          }));
        }

        const [dbArticles, dbPlaylists, dbProgress, dbQueue] =
          await Promise.all([
            localDB.getArticles(),
        ...
        if (dbArticles.length === 0) {
          ...
          await Promise.all(
            seedArticles.map((art) => localDB.saveArticle(art)),
          );
          ...
          const seedPlaylists: Playlist[] = [
            {
              ...
              description:
                "A balanced mix of technology, sustainability, and wellness.",
          ...
          await Promise.all(
            seedPlaylists.map((pl) => localDB.savePlaylist(pl)),
          );
  ...
  const addArticleText = useCallback(
    async (title: string, text: string) => {
      setIsLoading(true);
      try {
        const summaryResult = await ApiService.summarizeText(
          text,
          title,
  ...
  const importArticleUrl = useCallback(
    async (url: string) => {
      setIsLoading(true);
      try {
        const extractResult = await ApiService.extractUrl(
          url,
          preferencesRef.current,
        );
        const newArticle: Article = {
  ...
  const deleteArticle = useCallback(
    async (id: string) => {
      ...
      if (
        playbackStateRef.current.currentArticleId === id &&
      ...
      setPlaylists((prev) =>
        prev.map((pl) => {
          if (pl.articleIds.includes(id)) {
            const updated = {
              ...pl,
              articleIds: pl.articleIds.filter((aid) => aid !== id),
            };
      ...
      setPlaybackState((prev) => ({
        ...
        currentArticleId:
          prev.currentArticleId === id ? null : prev.currentArticleId,
  ...
  const downloadArticleAudio = useCallback(
    async (id: string) => {
      ...
      if (cached) {
        setArticles((prev) =>
          prev.map((art) =>
            art.id === id ? { ...art, isDownloaded: true } : art,
          ),
        );
        return;
      }
      ...
      try {
        ...
        const base64 = await ApiService.generateTTS(
          article.summary,
          article.voiceName,
          1.0,
        );
  ...
  const updatePlaylistDetails = useCallback(
    async (playlistId: string, name: string, description?: string) => {
      ...
      setPlaylists((prev) =>
        prev.map((p) => (p.id === playlistId ? updatedPl : p)),
      );
  ...
  const addArticleToPlaylist = useCallback(
    async (playlistId: string, articleId: string) => {
      ...
      await localDB.savePlaylist(updatedPl);
      setPlaylists((prev) =>
        prev.map((pl) => (pl.id === playlistId ? updatedPl : pl)),
      );
  ...
  const removeArticleFromPlaylist = useCallback(
    async (playlistId: string, articleId: string) => {
      const playlist = playlists.find((pl) => pl.id === playlistId);
      if (!playlist) return;
      const updatedPl = {
    ...
    (article: Article) => {
      if (typeof window === "undefined" || !("speechSynthesis" in window)) {
        setPlaybackState((prev) => ({
          ...prev,
          isPlaying: false,
          playbackError: "Audio playback unavailable.",
        }));
      ...
      try {
        ...
        setPlaybackState((prev) => ({
          ...prev,
          isPlaying: false,
          playbackError: "Audio speech synthesis failed.",
        }));
  ...
  const togglePlayPause = useCallback(() => {
    if (
      isSpeechSynthesisActiveRef.current &&
      ...
      try {
        ...
        if (!audioObj) {
          ...
          if (cachedBase64) {
            ...
            try {
              const liveBase64 = await ApiService.generateTTS(
                article.summary,
                article.voiceName,
                1.0,
              );
