...
export const QueuePanel: React.FC = () => {
  const { playbackState, articles, playArticle, removeFromQueue, clearQueue, reorderQueue, triggerHaptic } = useApp();
  const [draggedIndex, setDraggedIndex] = useState<number | null>(null);

  const { queue, currentArticleId } = playbackState;
  const queueArticles = queue.map(id => articles.find(a => a.id === id)).filter(Boolean) as Article[];
  ...
  return (
    <div id="queue-panel-container" className="max-w-xl mx-auto p-4 md:p-6 text-zinc-900 dark:text-white pb-32">
      <div className="flex justify-between items-center mb-6 border-b border-zinc-200 dark:border-zinc-900 pb-4">
        <div>
          ...
          <p className="text-zinc-600 dark:text-zinc-400 text-xs sm:text-sm mt-1">
            Drag vertical handle to reorder, or <span className="font-semibold text-emerald-600 dark:text-emerald-400">swipe left</span> to delete tracks.
          </p>
        </div>
        {queue.length > 0 && (
          <button
      ...
      {queueArticles.length === 0 ? (
        <div className="bg-white/60 dark:bg-zinc-900/20 border border-zinc-200 dark:border-zinc-900 rounded-2xl py-12 px-6 text-center shadow-sm">
          ...
          <h4 className="font-semibold text-zinc-700 dark:text-zinc-400">Queue is empty</h4>
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-red-500 dark:bg-red-600/90 rounded-xl px-5 flex items-center justify-end text-white font-medium text-xs gap-2 shadow-inner">
                    ...
                    <span className="font-mono uppercase text-[10px] tracking-wider font-bold">Delete</span>
                  </div>
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <h4 className={`font-semibold text-sm truncate mt-1 ${isCurrent ? 'text-emerald-600 dark:text-emerald-400' : 'text-zinc-800 dark:text-zinc-200'}`}>
                          {art.title}
                        </h4>
